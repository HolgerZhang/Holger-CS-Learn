==================================================================================
工程名称：Exam7_3-20201129
版权所有：SUDA-ARM(sumcu.suda.edu.cn)
版本更新：版本更新：20180810-20201129
本程序实现了：(1)每1S红灯闪烁一次，并通过串口UARTC（板上标识为uart2）输出红灯亮暗状态以及闪烁次数。
			  (2)每隔1S，电路板通过串口UARTA（板上标识UART0）接收一个字符串数据‘Assembly call  c's uart2!’并回发该字符串(波特率115200)。
	          (3)用户在上位机使用串口调试工具，实现通过串口UARTA（板上标识UART0）向开发套件的串口模块发送一个字符串的数据(波特率115200)。
==================================================================================         
