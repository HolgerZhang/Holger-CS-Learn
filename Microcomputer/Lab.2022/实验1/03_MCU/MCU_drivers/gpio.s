...
//（3）获得待操作端口时钟的RCC->AHB2ENR寄存器（不属于内部寄存器，看作是外部存储空间）的地址
      ldr r7,=RCC_AHB2ENR_BASE     //r7=RCC->AHB2ENR寄存器基地址（RCC_AHB2ENR_BASE为立即数，立即数寻址）
      ldr r5,[r7]                  //r5=RCC->AHB2ENR寄存器中的内容【寄存器间接寻址】
      mov r6,#1                    //r6=1
